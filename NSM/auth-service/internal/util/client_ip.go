...
// called from internal/handler/http, and every request-handling call site
// already reaches this package already (util.Claims,
// util.RequestIDFromContext) — threading a trusted-proxy list through
// every handler struct's constructor instead would touch several files
// for a value that is fixed for the lifetime of one process, set exactly
// once, at startup, by the composition root (cmd/server/main.go /
// test/e2e's own harness).
var (
...
// request actually from" — replacing auth_handler.go's old clientIP, which
// returned either the raw X-Forwarded-For header value unconditionally
// (trivially spoofable by any direct, unproxied client — rotate the header
// per request to look like a fresh IP, or set it to a victim's address to
// frame them) or, with no such header, r.RemoteAddr verbatim, including
// its :port suffix — which every caller ultimately stores in an
// inet-typed Postgres column (sessions, login_history, audit_logs), and
// which that column type rejects outright.
//
...
// (port stripped), full stop, regardless of what headers it carries — the
// only unconditionally safe behavior for a deployment that hasn't stated
// otherwise.
//
